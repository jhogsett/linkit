...
class EffectsProcessor
{
  ...
  void begin(Buffer *buffer, BlinkEffects *blink_effects, BreatheEffects *breathe_effects);
...
void EffectsProcessor::begin(Buffer *buffer, BlinkEffects *blink_effects, BreatheEffects *breathe_effects){
  this->buffer = buffer;
...
void EffectsProcessor::start_blinking_r() {
  buffer->get_effects_buffer()[0] = BLINK_ON_1 + random((BLINK_ON_6 - BLINK_ON_1) + 1);
}
...
void EffectsProcessor::start_effect_r() {
  buffer->get_effects_buffer()[0] = random_effect();
}
