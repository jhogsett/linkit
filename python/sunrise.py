...
def loop():
    ...
    try:
        ...
        sunrise_time = int(wx.daily_sunrise(data))

#        sunrise_time = current_time

        print "Sunrise: " + wx.format_unix_timestamp(sunrise_time)
	...
	if last_command_time == sunrise_time:
	...
	# sunrise_time is in the past but doesn't match
        # the last time the command was sent, so sent it
        print "doing command ..."
        do_command()

	# record that it was sent
        last_command_time = sunrise_time
